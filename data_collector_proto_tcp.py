...
def _extract_host_port(remote: str):
    """Return (host, port or None) from OpenVPN 'remote' field.
    ...
    if not remote:
        return remote, None
    # Strip protocol prefix: udp, udp4, udp6, tcp, tcp4, tcp6
    remote = re.sub(r'^(?:udp|tcp)(?:4|6)?(?:-server)?:', '', remote)
    # [IPv6]:PORT
    m = re.match(r'^\[(.+)\]:(\d+)$', remote)
    if m:
        return m.group(1), m.group(2)
    # IPv4:PORT
    m = re.match(r'^([0-9]{1,3}(?:\.[0-9]{1,3}){3}):(\d+)$', remote)
    if m:
        return m.group(1), m.group(2)
    # Host without port
    return remote, None
...
class VPNDataCollector(object):
    ...
    def collect_data(self, vpn, gi):
        ...
        if connection.is_connected():
            ...
            # Tolerant version parsing: extract MAJOR.MINOR[.PATCH] and ignore distro suffixes (e.g., -0.2025...el10_0)
            version_str = None
            if release:
                m = re.search(r'\b(\d+)\.(\d+)(?:\.(\d+))?\b', release)
                if m:
            ...
            try:
                if version_str:
                    ...
                    logging.warning("openvpn-monitor: cannot parse OpenVPN version from release=%r; defaulting to 0.0.0", release)
                    version = semver.VersionInfo.parse("0.0.0")
            except ValueError:
                logging.warning("openvpn-monitor: invalid version %r; defaulting to 0.0.0", version_str)
                version = semver.VersionInfo.parse("0.0.0")
            vpn['release'] = release
            vpn['version'] = version
    ...
    def parse_status(self, data, version, gi):
        ...
        for line in data.splitlines():
            ...
            if client_section:
                ...
                remote, port = _extract_host_port(remote_str)
                try:
                    remote_ip_obj = ip_address(remote) if remote else None
                except ValueError:
                    remote_ip_obj = None
                session['remote_ip'] = remote if remote else ''
                session['remote_ip_obj'] = remote_ip_obj
                if port:
                    session['port'] = int(port)
                else:
                    session['port'] = ''

                # Use a local variable for remote IP object and guard against empty strings
                _rip = session.get('remote_ip_obj')

                if _rip is None:
                    session['location'] = 'unknown'
                elif _rip.is_private:
                    session['location'] = 'RFC1918'
                elif _rip.is_loopback:
                    session['location'] = 'loopback'
                else:
                    try:
                        if gi:
                            gir = gi.city(str(_rip))
